...
- **Surprisingly popular** (§17.2, `surprisinglyPopular.ts`): `lean(pct)` is TRUE above 50, FALSE below, null at 50, read from the student's own first number (`first_pct ?? blind_pct`, `ownPct` in the machine) rather than the engine number, since a consider-the-opposite blend of two mirror numbers sits at exactly 50; `actualTruePct` over leaners, `predictedTruePct` the mean prediction (null below 3 predictions); `answer = actual > predicted` (null on a tie or missing side). `spInsight` = own lean equals the reference (outcome on STEM, `sp_answer` on humanities) and the expected majority differs from it.
...
| resolved | outcome, calibration before/after, persuasion, leaderboard top; own SP insight, steelman fidelity (also on a humanities question when one was written — it was graded live; the leaderboard mean stays over resolved STEM questions), contrarian credit; anonymous argument pairs to compare (§17.9b) | belief-map row, final histogram, leaderboard, SP insight rate, top arguments by pairwise strength, cascade comparison |
...
Every flag-gated field is null / empty with its flag off, so the P0 view is unchanged. Argument pairs carry opaque submission ids and texts only; a tally for a side never leaves the server (`argumentVotersPct` is a percentage; each top argument's `comparisons` is its sample size, teacher dashboard only, never on a phone or the projector).
...
| `POST /api/questions/:id/socrates` | teacher | `{}` → `SocratesResult` (§17.4; 409 `too_early`; open mode or no groups → `{ skipped: true }`; not flag-gated, the dashboard is). Idempotent: a group that already has one question per speaker is left alone; groups are re-read after the model calls and a group whose turn order changed under the run (a snapshot recompute) is composed once more from its fresh order, else counted in `stale` |
...
| Generator | P1 / §17.3 | medium, 25 s deadline | topic → 5, or cluster → 1–3 `{ text, correctAnswer, misconception, sourceClusterIndex }` | canned demo list (topic) / label-derived candidates with the answer unset (cluster) |
...
**Measured Sep 20, 2026** (local Supabase, `claude-opus-5`, `npm run agent:eval`, 10 cases each, 10-way concurrency): proposer 10/10 (p95 3.5 s), clusterer 8/10 (p95 5.9 s), Socrates 10/10 (p95 4.8 s), steelman 9/10 (p95 4.4 s), generator 10/10 at effort `medium` with `GENERATOR_DEADLINE_MS = 25_000` (p95 19.8 s; at the 8 s deadline it was 2/10 on `high` and 3/10 on `medium`, all timeouts on the five-proposition topic kind). The generator is teacher-triggered and out of band (its routes allow 30 s), so it is the one agent that does not use the 8 s deadline. Cluster-kind candidates are ordered stated-misconception (FALSE) first, then by cluster size, which is the plan's "largest wrong cluster first".
...
14. **§17 extensions are sub-steps and flags, never phases.** `PHASES` is positional and the machine indexes into it, so every §17 screen is decided server-side from submission state inside an existing phase and gated by `sessions.features` (all off by default, so the P0 path is unchanged). Cascade is per question, the open question is a `mode`, and everything lands in one additive migration (`0002_extensions.sql`).
15. **Consider the opposite (§17.1) blends to a multiple of 5 with a .5 rounding toward 50**, since beliefs are integer step 5 and the point of the exercise is less confidence. The dashboard stat reports the unsigned mean `|first − blind|` as "moved the first number N points on average" rather than "pulled confidence back": a second number on the same side pushes confidence out, and the stat must not misname that.
16. **Surprisingly popular (§17.2) reads the lean from the student's own first number, excludes 50 ("Unsure") from the denominator, and needs three predictions**; with §17.1 on, a blend of two mirror numbers sits at exactly 50 and would drop the student. The SP answer is a humanities winner and a STEM reveal only; it never changes the market or the scores.
17. **The open question (§17.3) is a third `mode` that goes `snapshot → resolved` and sharpens into candidates the teacher edits and adds.** The agent never inserts a question and the phone never shows a price for free text, so the "largest wrong cluster → proposition → market" sequence keeps a human between the clusterer and the next market.
18. **Socrates (§17.4) is one structured call per group, out of band, one question per speaker**, not a live agent loop: the question for speaker `i` is shown during turn `i` (index from the clock), so "stops when the turn timer ends" costs no server state and a slow model only delays the question to the next poll.
19. **Cascade (§17.5) is a second question with the same proposition and `cascade_mode`, compared at snapshot, not resolve**, so the pitch moment is not two phases late. Blind-phase trades are logged only for cascade questions (logging them everywhere adds three queries to every P0 submit under a burst). The phone copy is present tense ("As classmates answer, this is where the class stands right now.") rather than "Others have already answered", because the first submitter sees 50 before anyone has.
20. **Steelman fidelity (§17.7) and contrarian credit (§17.9a) never touch the displayed calibration.** Calibration stays the proper scoring rule; contrarian credit is added to the rank key only, steelman fidelity is the third key, a fallback grades `null` (neither reward nor penalty), and the steelman box stays open through `snapshot` and `structured` rather than strictly "before the structured round" so the untimed wait is usable.
21. **The arc and replay (§17.6, §17.8) are dashboard surfaces built from `phase_log` and trades**: x is seconds when the span is ≥ 5 s, else point index (the simulator drives a question in about two seconds); the replay is dashboard-only with anonymous captions (never a name, id, own number, or move size) and a JSON export, since a synchronized projector replay would be server state for one demo beat.
22. **Argument Elo (§17.9b) is Bradley–Terry with a fixed reference item** (Hunter's MM), so an unbeaten argument stays finite and an uncompared one sits at exactly 1; pairs are seeded per `(question, voter)` so the 2 s poll never reshuffles a pair under a thumb; the mind-changer graph needs P2 persistence and is cut.
